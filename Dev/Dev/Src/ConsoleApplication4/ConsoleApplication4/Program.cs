...
namespace ConsoleApplication4
{
    ...
    class Program
    {
        ...
        {
            //string cc = "6282-0001-2384-2342";
            //string a = ValidateCardNumber(cc);
            //bool c = IsValidCardType(CleanCreditCardNumber(cc));

            //CreditCardTypeType? x = CreditCardUtility.GetCardTypeFromNumber(CleanCreditCardNumber(cc));

            //DateTime date =  new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1).AddMonths(-(1 - 1));
            //List<string> list = new List<string>();
            //list.Add("asil");
            //list = list.Where(item => item == "asil").ToList();
            ...
            //for (int a = 0; a < 256; a++)
            //{
            //    PingReply reply = pingSender.Send(string.Format("192.168.34.{0}", a), timeout, buffer, options);
            ...
            //List<int> aa = new List<int>() { 1, 2, 3, 4, 5, 6, 7 };
            //XmlSerializer s = new XmlSerializer(typeof(List<int>));
            //s.Serialize(sw, aa);

            //string serializedXml = sw.ToString();

            //List<int> bb = (List<int>)s.Deserialize(new StringReader(serializedXml));


            //Task OneWayMessageManagerTaks = new Task(() =>
            //{
            //    int a = 0;
            //    int b = 1000;
            //    int c = b / a;
            ...
            // object a = GetLockObject(aa,10);

            // object b = GetLockObject(aa, 12);
            //PickZone a  = PickZone.CasePick2A;
            //if ((byte)a > 0 && (byte)a < 100)
            //    Console.WriteLine("sda");


            //MailMessage tMailMessage = new MailMessage("laxmi-m@iherb.com", "esi@iherb.com,amin-p@iherb.com,mohammad-b@iherb.com,joseph-d@iherb.com,chinmaya-n@iherb.com,eiman@iherb.com,brad-o@iherb.com,gary-m@iherb.com");

            //tMailMessage.Subject = "Lunch on Friday the 17th Nov. Plz join at 12.00PM at Dave&Busters, Thanks!!!!!";
            //tMailMessage.From = new MailAddress("laxmi-m@iherb.com");
            //StringBuilder str = new StringBuilder();
            ////str.Append("Everyone knows who is responsible about mistake so:");
            ////str.AppendLine("I don't blame anyone else for my mistake, apologize for my mistake and try to correct my mistake as soon as possible.");
            ////str.Append("and finally i want to invite anyone who works in Irvine office to lunch");
            //str.AppendLine("Because I made such a big mistake I would like to take all of you out to lunch.");
            //str.AppendLine("");
            //str.AppendLine("Thanks,");
            //str.AppendLine("Laxmi");

            //tMailMessage.Body = str.ToString();

            //tMailMessage.IsBodyHtml = false;

            //SmtpClient client= new SmtpClient("192.168.34.25");
            //client.Send(tMailMessage);
            //double a = 10.1;
            //decimal amount = Convert.ToDecimal(a.ToString("0.00"));
            ...
            //try
            //{
            //OneWayMessageManagerTaks.Start();
            //}
            //catch (Exception ex)
            //{
